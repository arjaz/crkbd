#include QMK_KEYBOARD_H
#include <keymap_ukrainian.h>
#include "features/achordion.h"
#include "features/socd_cleaner.h"

#define _ALPHA 0
#define _UKR 1
#define _NAV1 2
#define _NAV2 3
#define _GAME 4
#define _SYM 5
#define _NUM 6

#define AR_J KC_J
#define AR_G KC_G
#define AR_M KC_M
#define AR_P KC_P
#define AR_V KC_V
#define AR_ASTR KC_ASTR
#define AR_MINS KC_MINS
#define AR_QUOT KC_QUOT
#define AR_EQL KC_EQL
#define AR_HASH KC_HASH
#define AR_C KC_C
#define AR_S KC_S
#define AR_N LCTL_T(KC_N)
#define AR_T LALT_T(KC_T)
#define AR_H LGUI_T(KC_H)
#define AR_K KC_K
#define AR_COMM KC_COMM
#define AR_A LGUI_T(KC_A)
#define AR_E LALT_T(KC_E)
#define AR_I LCTL_T(KC_I)
#define AR_X KC_X
#define AR_F KC_F
#define AR_L LT(_NAV1, KC_L)
#define AR_D KC_D
#define AR_W KC_W
#define AR_DOT KC_DOT
#define AR_U LT(_NAV2, KC_U)
#define AR_O KC_O
#define AR_Y KC_Y
#define AR_B KC_B
#define AR_R KC_R
#define AR_Q KC_Q
#define AR_QU LT(0, KC_LANGUAGE_9)
#define AR_Z KC_Z
#define AR_BSPC KC_BSPC
#define AR_SYM MO(_SYM)
#define AR_NUM MO(_NUM)
#define AR_LSFT KC_LSFT
#define AR_SPC KC_SPC
#define AR_F5 LGUI_T(KC_F5)

socd_cleaner_t socd_v = {{KC_W, KC_S}, SOCD_CLEANER_LAST};
socd_cleaner_t socd_h = {{KC_A, KC_D}, SOCD_CLEANER_LAST};

uint16_t achordion_streak_timeout(uint16_t tap_hold_keycode) {
    return ACHORDION_STREAK_TIMEOUT;
}

uint16_t achordion_timeout(uint16_t tap_hold_keycode) {
    return ACHORDION_TIMEOUT;
}

bool achordion_chord(uint16_t tap_hold_keycode,
                     keyrecord_t* tap_hold_record,
                     uint16_t other_keycode,
                     keyrecord_t* other_record) {
    uint16_t raw_key = tap_hold_keycode & 0xFF;
    uint16_t raw_other = other_keycode & 0xFF;
    if (
        ((raw_key == KC_S || raw_key == KC_T || raw_key == KC_W)
         && raw_other == KC_H)
        || (raw_key == KC_N && raw_other == KC_G)
        )
        return false;
    return true;
}

void matrix_scan_user(void) {
    achordion_task();
}

enum {TD_T_QUOT, TD_U_BSLS, TD_P_LBRC};
enum macros_keycodes {
    SCROLL_LOCK_TG_CYRILLIC = SAFE_RANGE
};

uint16_t get_tapping_term(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
    case TD(TD_T_QUOT):
    case TD(TD_U_BSLS):
    case TD(TD_P_LBRC):
        return 200;
    default:
        return TAPPING_TERM;
    }
}

layer_state_t layer_state_set_user(layer_state_t state) {
  socd_cleaner_enabled = IS_LAYER_ON_STATE(state, _GAME);
  return state;
}

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    if (!process_socd_cleaner(keycode, record, &socd_v)) return false;
    if (!process_socd_cleaner(keycode, record, &socd_h)) return false;
    if (!process_achordion(keycode, record)) return false;
    switch (keycode) {
    case SCROLL_LOCK_TG_CYRILLIC:
        if (record->event.pressed) {
            tap_code(KC_SCROLL_LOCK);
            layer_invert(_UKR);
        }
        break;
    case AR_QU:
        if (record->event.pressed) {
            if (record->tap.count == 0) {
                tap_code16(KC_Q);
            } else {
                tap_code16(KC_Q);
                unregister_mods(MOD_MASK_SHIFT);
                tap_code16(KC_U);
            }
        }
        return false;
    }
    return true;
}

tap_dance_action_t tap_dance_actions[] = {
    [TD_T_QUOT] = ACTION_TAP_DANCE_DOUBLE(KC_T, KC_QUOT),
    [TD_U_BSLS] = ACTION_TAP_DANCE_DOUBLE(KC_U, KC_BSLS),
    [TD_P_LBRC] = ACTION_TAP_DANCE_DOUBLE(KC_P, KC_LBRC),
};

#include <g/keymap_combo.h>

bool combo_should_trigger(uint16_t combo_index, combo_t *combo, uint16_t keycode, keyrecord_t *record) {
    if (layer_state_is(_NAV2) && combo_index != COMBO_NAVIGATION)
        return false;
    if (layer_state_is(_SYM))
        return false;
    switch (combo_index) {
    case COMBO_Z:
    case COMBO_Q:
        if (!layer_state_is(_ALPHA))
            return false;
        break;
    case COMBO_CYRILLIC:
        if (!(layer_state_is(_ALPHA) || layer_state_is(_UKR)))
            return false;
        break;
    case COMBO_GAMING:
        if (!(layer_state_is(_ALPHA) || layer_state_is(_GAME)))
            return false;
        break;
    }

    return true;
}

const key_override_t *key_overrides[] = {
    &ko_make_with_layers(MOD_MASK_SHIFT, KC_DOT, S(KC_SCLN), 1 << _ALPHA),
    &ko_make_with_layers(MOD_MASK_SHIFT, KC_COMM, KC_SCLN, 1 << _ALPHA),
    &ko_make_with_layers(MOD_MASK_SHIFT, KC_ASTERISK, KC_EXLM, 1 << _ALPHA),
};

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_ALPHA] = LAYOUT_split_3x5_3
    (AR_V,   AR_W,    AR_G,    AR_M,    AR_J,
     AR_EQL, AR_MINS, AR_QUOT, AR_ASTR, AR_HASH,

     AR_S,    AR_N, AR_T, AR_H, AR_K,
     AR_COMM, AR_A, AR_E, AR_I, AR_C,

     AR_F,   AR_P, AR_D, AR_L, AR_X,
     AR_DOT, AR_U, AR_O, AR_Y, AR_B,

     AR_SYM,  AR_R,   AR_BSPC,
     AR_LSFT, AR_SPC, AR_NUM
     ),

    [_UKR] = LAYOUT_split_3x5_3
    (KC_Q, KC_W,          KC_E, KC_R, TD(TD_T_QUOT),
     KC_Y, TD(TD_U_BSLS), KC_I, KC_O, TD(TD_P_LBRC),

     KC_A, LCTL_T(KC_S), LALT_T(KC_D), LGUI_T(KC_F), KC_G,
     KC_H, LGUI_T(KC_J), LALT_T(KC_K), LCTL_T(KC_L), KC_SCLN,

     KC_Z, KC_X,                        KC_C,    LT(_NAV1, KC_V), KC_B,
     KC_N, LT(_NAV2, KC_M), KC_COMM, KC_DOT,   KC_RBRC,

     KC_TRNS, KC_TRNS, KC_TRNS,
     KC_TRNS, KC_TRNS, KC_TRNS
     ),

    [_NAV1] = LAYOUT_split_3x5_3
    (KC_NO, KC_NO,   KC_NO,   KC_NO, KC_NO, KC_NO, KC_NO,   KC_NO,   KC_NO,   KC_NO,
     KC_NO, KC_LCTL, KC_LALT, KC_NO, KC_NO, KC_NO, KC_VOLD, KC_UP,   KC_VOLU, KC_NO,
     KC_NO, KC_NO,   KC_NO,   KC_NO, KC_NO, KC_NO, KC_LEFT, KC_DOWN, KC_RGHT, KC_NO,
     KC_NO,   KC_MS_BTN1, KC_MS_BTN2,
     KC_PGDN, KC_PGUP,    KC_NO
     ),

    [_NAV2] = LAYOUT_split_3x5_3
    (KC_NO,      KC_NO,   KC_NO,   KC_NO,   KC_NO,   KC_NO, KC_NO,   KC_NO,   KC_NO,   KC_NO,
     KC_NO,      KC_WH_D, KC_MS_U, KC_WH_U, KC_HOME, KC_NO, KC_VOLD, KC_UP,   KC_VOLU, KC_NO,
     KC_MS_BTN3, KC_MS_L, KC_MS_D, KC_MS_R, KC_END,  KC_NO, KC_LEFT, KC_DOWN, KC_RGHT, KC_NO,
     KC_NO,   KC_MS_BTN1, KC_MS_BTN2,
     KC_PGDN, KC_PGUP,    KC_NO
     ),

    [_GAME] = LAYOUT_split_3x5_3
    (KC_Q, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I,    KC_O,   KC_P,
     KC_A, KC_S, KC_D, KC_F, KC_G, KC_H, KC_J, KC_K,    KC_L,   LT(_NUM, KC_SCLN),
     LCTL_T(KC_Z), LALT_T(KC_X), KC_C, KC_V, LT(_NUM, KC_B), KC_N, KC_M, KC_COMM, KC_DOT, KC_RBRC,
     KC_LALT, KC_SPC,  KC_LSFT,
     KC_TRNS, KC_TRNS, KC_TRNS
     ),
    
    [_SYM] = LAYOUT_split_3x5_3
    (KC_NO,  KC_LBRC, KC_RBRC, KC_NO,    KC_NO, KC_EQL,  KC_TRNS,  KC_SLASH, KC_ASTERISK, KC_HASH,
     KC_LT,  KC_LPRN, KC_RPRN, KC_GT,    KC_NO, KC_BSLS, KC_TILDE, KC_QUES,  KC_AMPR,    KC_EXLM,
     KC_DLR, KC_LCBR, KC_RCBR, KC_GRAVE, KC_NO, KC_TRNS, KC_AT,    KC_PERC,  S(KC_BSLS), KC_CIRC,
     KC_TRNS, KC_TRNS, KC_TRNS,
     KC_RALT, KC_TRNS, KC_TRNS
     ),

    [_NUM] = LAYOUT_split_3x5_3
    (KC_NO, KC_7, KC_8, KC_9, KC_NO, KC_DEL,  KC_F9, KC_F10, KC_F11, KC_F12,
     KC_NO, KC_4, KC_5, KC_6, KC_NO, KC_TRNS, AR_F5, KC_F6,  KC_F7,  KC_F8,
     KC_NO, KC_1, KC_2, KC_3, KC_NO, KC_TRNS, KC_F1, KC_F2,  KC_F3,  KC_F4,
     KC_TRNS, KC_0, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS
     )
};
